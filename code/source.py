...
def findcount(val :str, sentences :list):
    '''
    Finds count of the provided word in the list of sentences.
    '''
    count = 0
...
def tokenize(file_path :str):
    '''
    Tokenize and return a list of the sentences in the file path
    '''
    lst = []
    f = open(file_path, 'r')
    for x in f:
...
def Perplexity(sentence :str, n: int):
    '''
    Calculates the perplexity of the model 
    using specified formula
    '''
    probability = SentenceProb(sentence)
    probability = 1 / probability
    perplex = probability ** (1/n)
    return perplex
...
print(f'Perplexity of "Allen is a member of the Board": {Perplexity("Allen is a member of the Board", 2)}\n')
